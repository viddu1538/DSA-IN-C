...
{
	node *z,*n;  // z is for loop and n is for new node at last 
	n=(node *)malloc(sizeof(node));
	printf("\n enter data for the new node :");
	scanf("%d",&n->data);
	n->next=NULL; // since it will be the last node so it hold null in next part
	z=head; // initilazation of z is important if not then the loop start from random memory address 
	while(z->next!=NULL)
	{
		z=z->next;    // traversing to the last node 
	}
	z->next=n;  // attatch new node 
	return head;				
}
